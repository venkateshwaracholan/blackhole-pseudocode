...
public class MergeIntervals {
    ...
    //APPROACH 1 sortBy start + arraylist last + add first upfront + update max end in merge=> sort by start, ans.add(in[0]),
    //                  for(i=1,len) last = ans.getlast   if( last < interval) then add interval, else merge, simple
    ...
    //APPROACH 2 sort by end + rev Ite + add last upfront + update start in merge => 
    ...
    //APPROACH 3 sort by start end sep+ Ite + two pointers =>  sep start and end in int[], fill and sort, then start 2 pointer loop
    //                  for(i=0j=0,i<n;i++) if i reaches end or end[i]<start[i+1] meaning, next>cur we add start[j],end[i] to list and movve jto i+1
