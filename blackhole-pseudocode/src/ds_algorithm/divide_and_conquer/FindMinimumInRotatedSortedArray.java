...
public class FindMinimumInRotatedSortedArray {
    ...
    */
    ...
    /*
    * ONE LINER => Binary Search r‑anchored: set l=0,r=n-1, if nums[l] ≤ nums[r] (no rotation) return nums[0](first elem), else binary search anchored on r — compare mid with nums[r]: if nums[mid] > nums[r] → l=mid+1 (min in right half), else r=mid (min in left half including mid), repeat until l=r (meaning we always keep min in [l,r]), return nums[l] (min) → O(log n), O(1).
    *
    ...
    */
    ...
    /*
    * ONE LINER => Unsafe Binary Search: set l=0,r=n-1, if nums[l] ≤ nums[r] (no rotation) return nums[0](first elem), else use l ≤ r and mid+1 check — if nums[mid]>nums[mid+1] pivot found return nums[mid+1], else if nums[mid]>nums[r] → l=mid+1 else r=mid; repeat until found → O(log n), O(1); risk of index errors if not mid+1 check.
    *
    ...
    */
    ...
    /* ONE LINER => Unsafe l‑variant: set l=0,r=n-1, if nums[l] ≤ nums[r] (no rotation) return nums[0](first elem), else use l ≤ r with mid+1 check — if nums[mid]>nums[mid+1] return nums[mid+1], else if nums[l]<nums[mid] → l=mid+1 else r=mid; repeat until found, return nums[l] → O(log n), O(1).risk of index errors if not mid+1 check.
    */
