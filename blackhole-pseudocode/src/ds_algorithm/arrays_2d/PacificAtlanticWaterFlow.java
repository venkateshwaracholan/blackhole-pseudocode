...
public class PacificAtlanticWaterFlow {
    ...
    // exactly same as above using 2 sets
    public List<List<Integer>> pacificAtlantic2(int[][] heights) {
        ...
        List<List<Integer>> ans = new ArrayList();
        for(int i:atl)
                if(pac.contains(i)) ans.add(Arrays.asList(i/n,i%n));
        return ans;
    }

    public void pacificAtlantic(int[][] heights, int i, int j, int prev, Set<Integer> dp) {
        if(i==-1||i==heights.length||j==-1||j==heights[0].length||dp.contains(i*heights[0].length+j)||heights[i][j]<prev) return;
        dp.add(i*heights[0].length+j);
        pacificAtlantic(heights,i-1,j,heights[i][j],dp);
        pacificAtlantic(heights,i+1,j,heights[i][j],dp);
        pacificAtlantic(heights,i,j-1,heights[i][j],dp);
        pacificAtlantic(heights,i,j+1,heights[i][j],dp);
    ...
    public List<List<Integer>> pacificAtlantic4(int[][] heights) {
        ...
        bfs2(heights,m,n,pq,pac,dir);
        bfs2(heights,m,n,aq,atl,dir);
        List<List<Integer>> ans = new ArrayList();
        for(int i=0;i<m;i++)
            for(int j=0;j<n;j++)
                if(pac[i][j]&&atl[i][j]) ans.add(Arrays.asList(i,j));
    ...
    public void bfs2(int[][] heights, int m, int n,Queue<int[]> q, boolean[][] dp, int[][] dir) {
        while(!q.isEmpty()){
            int[] x = q.poll();
            for(int k=0;k<dir.length;k++){
