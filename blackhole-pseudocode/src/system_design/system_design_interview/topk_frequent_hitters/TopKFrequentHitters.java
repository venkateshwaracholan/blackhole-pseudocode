...
public class TopKFrequentHitters {
    ...
    return list of heavy hitters as close to real time, eg, list of most viewed videos for several minutes
    ...
    LoadBalancer splits events to multiple hosts and then then overall events are aggregated into a single storage host
    but we are using too much memory
    so partitioning or sharding can help data fit in memory of single host
  
    Data partitioner
        responsible for routing videoId to its own host, then compute local topK in each hosts, then we need to combine the topK from many hosts
        procesor host only passes k entries to storage host
        merge K sorted List can help
    
    streaming data is unbounded or inf, users click videos every sec, 
    so processor hosts can accumulate data only for a short period of time(ex: 1 minute) then flush to storage host
    rememeber we can only store data for top k heavy hitters and info of others are lost, as we cannot afford to store data abt every video in memory
    ...
    lets store all data to disk and use batch processing to compute topk with Map reduce
    ...
    API gateway
        ...
        single entrypoint, aggregates data on the fly or via background process that process logs and flushes based on time or size, serialize data in compact binary format AVRO
        log generation for monitoring, audit and billing
        api gateway is a widely spread paractice 
        ...
        so, we create a buffer of memory, read log entries, create freq table, buffer is fixed size, flushed when full or time out
    ...
    Fast Path:
        we calculate heavy hitters approximately and results are available within seconds
    fast processor
        ...
        as count min sketch produces aprox results we may lose data in rare cases if host dies as long as slow path doesnt lose data
        and results are avaibale in some time. absense of replication simplifies the system, a good tradeoff to discuss 
        every sevral sec, fast processor flushes data to storage
        since count min sketch has fixed size, nothing stops us from aggregating over several minites, this will just delay final results, another tradeoff
    
    storage
        merging several count min sketch is simply summing  
        servce infront of DB, sql, nosql, stores topk list for some time interval, data replication is required
    ...
    this cluster may be big with thousands of hosts, they preaggregate data on each hosts for several seconds, so it decreased number of messages to kafka
    ...
    this is imortant when we deal with stream processing and data aggregation
    ...
    freq count mapreduce job
        ...
        record reader parses data and offer to mapper as key value pairs
        mapper takes the key value pairs and stores a intermediate pairs
        paritioner takes intermeidate pairs and assigns patitions to group all values of each key and make that all values go to same reducer
        partitoned data is written to local disk for each mapper
    ...
    API Gateway should expose topK operation, routes data retreival tasks to storage service which gets data from underlying database
    ...
    we can give back precisely for 1 hour list from slow path, but cant precisely give back for 2 hours by merging 2 1 hour lists(as  it wont be accurate)
    ...
    how big is k?
        ...
        but several thousands are probably okay, but tens of thousands may cause perfomance degradations
    ...
    can we use apache kafka and stream processing to solve this?
        yes and no, we can solve with kafka + spark on a single pipeline. but it does what we did basically : data partitioning  + in memeory aggregation
