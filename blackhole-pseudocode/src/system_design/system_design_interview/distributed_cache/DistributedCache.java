...
public class DistributedCache {
    ...
    maintaining list of cache servers
        1. config files + continuous deployment pipeline like chef / puppet to deploy file to every host
        2. file + object storage(s3) + serrvice hosts poll file periodically
        3. configuration service(zookeeper) - discover cache hosts and monitor health,  each cache hosts registers and send heartbeats
    ...
    leader election:
        configuration service(zookeeper, redis sentinel) - monitors leaders and followers, if lead died, it promotes follower to leader, its is a distibuted system of its own
            has odd number of nodes to achieve quorom writes easier
    ...
    Data Expiration - include time to live atributes to expire data, we can wither passively expire and when someone tries to access the itemis expired
            ...
            since billions of items are there, we cant iterate all items, so probablistic algorithms are used, random items are tested
    ...
    monitoring and logging - many teams can use, if they get perfomance degradation, tehy will come to us as potential sources, we shld be able to answer
    ...
    consistent hasing - problems 
            1. domino effect - when a server dies, its load is transferreed to next server and it dies causing chain reaction
